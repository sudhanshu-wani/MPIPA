...
def get_person_id(name):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT ROWID FROM Users WHERE Mname = ?", (name,))
...
def get_person_age(name):
    conn = sqlite3.connect('mydatabase.db')
    cursor = conn.cursor()
    cursor.execute("SELECT age FROM Users WHERE Mname = ?", (name,))
    result = cursor.fetchone()
    conn.close()
    return result[0] if result else None
...
def get_person_gender(name):
    conn = sqlite3.connect('mydatabase.db')
    cursor = conn.cursor()
    cursor.execute("SELECT gender FROM Users WHERE Mname = ?", (name,))
    result = cursor.fetchone()
    conn.close()
    return result[0] if result else None
		...
		for (top, right, bottom, left), name in zip(face_locations, face_names):
			...
			if(name != 'Unknown'):
				...
				if(name!=prename):
					...
					if person_id:
						message = f" Missing Person Found!!!\nName: *{name}*\nID: *{person_id}*\nAge: *{person_age}*\nGender: *{person_gender}*\nLoaction: Department of Computer Engineering, P.E.S. Modern College"
					else:
						message = f"{name} found at Department of Computer Engineering, P.E.S. Modern College"
        
					pywhatkit.sendwhatmsg_instantly("+918379975886",message)	
